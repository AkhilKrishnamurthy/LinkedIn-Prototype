...
class JobPostingDetails extends Component {
  ...
  async componentDidMount() {
    axios
      .get("http://localhost:3001/JobPostingHistory")
      .then(async response => {
  ...
  render() {
    ...
    let filteredProperties = this.state.postedJobs.filter(job => {
      ...
      return (
        <div key={index}>
          <div className="job-title">
            <b>
              <Link to="#" id={index} onClick={this.handleClick}>
                {job.jobTitle}
              </Link>
            </b>
          </div>
          <button
            className="btn btn-lg save-btn flt-right"
            id={index}
            onClick={() => this.handleApplyClick(job)}
          >
            Edit
          </button>
        ...
        <div>
          <div className="form-group form_group_home">
            <input
              ...
              className="form-control form-control-lg form_control_home_location col-xs-3"
            />
          </div>
